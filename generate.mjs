...
본문 규칙:
- 소제목은 \`## 1. ~\` 형식. **5~6개만 씁니다. 7개를 넘기지 마세요.**
  상위 노출 글은 소제목이 오히려 적고, 대신 한 소제목 아래 문단이 많습니다.
- **문장은 짧게 끊습니다.** 한 문장 40자 안팎, 한 문단 한 문장이 기본입니다.
  접속사로 잇지 말고 마침표로 끊고 줄을 바꾸세요. 상위 글의 절반이 한 문장짜리 문단입니다.
- **노린 검색어를 통째로 반복하지 마세요.** 상위 글의 71%가 검색어 원문을 한 번도 안 씁니다.
  대신 핵심 낱말 하나(예: 검색어가 "해고예고수당 조건"이면 \`해고예고수당\`)를
  본문 전체에 15~20회 자연스럽게 씁니다.
- 이미지가 들어갈 자리마다 \`![카드뉴스: 무엇을 보여줄지 한 줄]()\` 를 넣으세요.
  **총 14~16개**입니다. 개수를 억지로 채우지 말고 독자가 눈으로 쉬어갈 자리에 넣습니다.
  카드뉴스 파이프라인이 이걸 보고 만듭니다.
- 표는 마크다운 표로
